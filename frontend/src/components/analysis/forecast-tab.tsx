...
function ForecastChartItem({ forecast }: { forecast: any }) {
  ...
  const option = {
    tooltip: {
      ...
      borderColor: "#27272A",
      textStyle: { color: "#E4E4E7", fontSize: 11, fontWeight: "bold" },
    },
    grid: {
      left: "4%",
      right: "4%",
    ...
    xAxis: {
      ...
      axisLabel: { color: "#71717A", fontSize: 10, fontWeight: "bold" },
    },
    yAxis: {
      type: "value",
      axisLine: { lineStyle: { color: "rgba(255,255,255,0.06)" } },
  ...
  return (
    <div className="space-y-4">
      <div ref={chartRef} className="w-full h-80 bg-[#09090B] border border-[#27272A]/50 rounded-xl" />
      <div className="flex flex-wrap gap-6 justify-center text-xs text-zinc-500 font-mono pt-2">
        <div className="flex items-center gap-2">
          <span className="w-3 h-1.5 rounded bg-[#ea580c]" />
          <span>Expected Projection (yhat)</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-3 h-3 rounded bg-[#ea580c]/10 border border-[#ea580c]/20" />
  ...
  if (isLoading) {
    return (
      <div className="space-y-6 animate-pulse">
        <div className="h-16 rounded-xl bg-[#111217]/50 border border-[#27272A]" />
        <div className="h-96 rounded-xl bg-[#111217]/50 border border-[#27272A]" />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#27272A] pb-4">
        <div>
          <h2 className="text-base font-bold text-zinc-200">Prophet Predictive Forecasting</h2>
        ...
        <div className="flex items-center gap-3">
          {numericColumns.length > 0 && (
            ...
            >
              <SelectTrigger className="w-48 bg-[#111217]/50 border-[#27272A] text-zinc-300 text-xs font-bold rounded-lg">
                <SelectValue placeholder="Select Metric" />
              </SelectTrigger>
              <SelectContent className="bg-[#111217] border-[#27272A] text-zinc-350 text-xs font-bold">
          ...
          >
            <SelectTrigger className="w-32 bg-[#111217]/50 border-[#27272A] text-zinc-300 text-xs font-bold rounded-lg">
          ...
          <Button
            ...
            className="bg-gradient-to-r from-[#ea580c] to-[#f97316] hover:from-[#f97316] hover:to-[#ea580c] text-zinc-950 font-black text-xs uppercase tracking-wider px-4 py-2 rounded-lg hover:scale-[1.01] transition-all cursor-pointer shadow-md"
          >
            Generate Forecast
          </Button>
        </div>
      ...
      {forecast ? (
        <Card className="border-[#27272A] bg-[#111217] shadow-xl">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              <CardTitle className="text-xs uppercase tracking-wider font-bold text-zinc-250">
                Future Projection: <span className="text-[#ea580c]">"{forecast.metric_name}"</span>
              </CardTitle>
            </div>
            <Badge className="text-[9px] uppercase tracking-wider font-bold bg-zinc-950 text-zinc-400 border border-[#27272A] flex items-center gap-1.5 rounded-lg py-1 px-2.5">
        ...
        <Card className="border-[#27272A] bg-[#111217]/30 border-dashed py-16 text-center">
          <CardContent className="space-y-4">
            <LineChart className="w-12 h-12 text-zinc-650 mx-auto" />
            <div>
              <h4 className="font-bold text-zinc-300 text-sm uppercase tracking-wide">Forecasting unavailable</h4>
